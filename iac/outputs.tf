...
output "web_public_ip" {
  ...
  value = aws_eip.main_backend_web_eip[0].public_ip

  // This output waits for the Elastic IPs to be created and distributed
  depends_on = [aws_eip.main_backend_web_eip]
}
...
output "web_public_dns" {
  ...
  // We are grabbing it from the Elastic IP
  value = aws_eip.main_backend_web_eip[0].public_dns
...
output "lb_dns" {
  value       = aws_alb.main-backend-lb.dns_name
