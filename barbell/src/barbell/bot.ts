...
import type { ChannelType, HandlerInput, IO } from "./types/handlerInputs";
	...
	static fromSlackState(
		...
		if (state.type === "multi_static_select") {
			return new MultiSelectInput(
				name,
				state.selected_options.map((option: { value: string }) => option.value),
			);
		}
...
class ActionRunner {
	...
	constructor({
		...
		name: string;
		handler: (handlerInputs: HandlerInput) => Promise<void>;
	}) {
		if (name === INIT_MODAL_NAME) {
			throw new Error(
				"Invalid action name, this action name is reserved for the initial modal",
			);
		}
		this.name = name;
		this.io = {
			...
			output: {
				markdown: async (value: string): Promise<MarkdownOutput> => {
					...
					if (
						this.inputoutputs.find((inputoutput) => inputoutput.name === value)
					) {
						return this.inputoutputs.find(
							(inputoutput) => inputoutput.name === value,
						) as MarkdownOutput;
					}
	...
	public async run(
		...
		await Promise.all(
			this.inputoutputs.map((inputoutput) => inputoutput.render()),
		);
		if (buttonClick) {
			const button = this.inputoutputs.find(
				(inputoutput) => inputoutput.name === buttonClick.value,
			) as ButtonInput;
...
export class Action {
	...
	constructor({
		...
		if (name.length > 25) {
			throw new Error(
				`Action name cannot be longer than 25 characters (Action name: "${name}" has ${name.length} characters)`,
			);
		}
		if (name === INIT_MODAL_NAME || name === INIT_ACTION_ID) {
			throw new Error(
				"Invalid action name, please refrain from using a reserved action name",
			);
		}
		this.name = name;
		this.handler = handler;
	...
	async run(
		...
		}); // Allow everything to be stateless server-side
		if (inputs) {
