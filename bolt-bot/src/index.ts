...
bot.defineAction({
	...
	handler: async (payload: any) => {
		...
		const selectedOptions = await io.Input.static_select("Why do you keep pooping your pants:", channel, thread_ts, [
			{ text: { type: "plain_text", text: "1 day", emoji: true }, value: "1" },
			{ text: { type: "plain_text", text: "7 days ", emoji: true }, value: "7" },
			{ text: { type: "plain_text", text: "30 days", emoji: false }, value: "30" },
			{ text: { type: "plain_text", text: "90 days", emoji: false }, value: "90" },
		...
		await io.Input.rich_text("ahhh, you shat your pants!", channel, thread_ts, `You pooped your pants ${Math.round(Math.random() * 100)} times in the past ${lookbackperiod.days} days.`);
	},
});
